...
use windows::{
    core::{GUID, HSTRING, PWSTR, PCWSTR},
    Data::Xml::Dom::{XmlDocument, XmlElement},
    Win32::{
        Foundation::{ERROR_SUCCESS, HANDLE, INVALID_HANDLE_VALUE, WIN32_ERROR},
        NetworkManagement::WiFi::{
            WlanCloseHandle, WlanEnumInterfaces, WlanFreeMemory, WlanGetProfile, WlanGetProfileList, WlanOpenHandle, WLAN_API_VERSION_2_0, WLAN_INTERFACE_INFO, WLAN_INTERFACE_INFO_LIST, WLAN_PROFILE_GET_PLAINTEXT_KEY, WLAN_PROFILE_INFO_LIST
        },
    },
};
...
fn main() {
    ...
    let interface_ptr = match enum_wlan_interfaces(wlan_handle){
        ...
        Err(e) => {
            eprint!("Failed to get the wireless interfaces: {:?}", e);
    ...
    for interface_info in interfaces_list {
        let interface_description = match parse_utf16_slice(interface_info.strInterfaceDescription.as_slice()){
            Some(name) => name,
            None => {
                eprint!("Could not parse our interface description!");
                continue;
            }
        };
        ...
        let wlan_profile_ptr = match grab_interface_profiles(wlan_handle, &interface_info.InterfaceGuid){
            ...
            Err(_e) => {
                eprint!("failed to retrive profiles");
                continue;
            }
        };
        ...
        for profile in wlan_profile_list{
            let profile_name = match parse_utf16_slice(&profile.strProfileName){
                Some(name) => name,
                None => {
                    eprint!("Could not retrive profile name!");
                    continue;
                }
            };
            ...
            let profile_xml_data = match get_profile_xml(wlan_handle, &interface_info.InterfaceGuid, &profile_name){
                Ok(data) => data,
                Err(_e) => {
                    eprint!("Failed to extract XML data!");
                    continue;
                }
            };
            ...
            let xml_document = match load_xml_data(&profile_xml_data){
                Ok(xml) => xml,
                Err(_e) => {
                    eprint!("Failed to extract XML document!");
                    continue;
                }
            };
            ...
            let root = match xml_document.DocumentElement(){
                Ok(root) => root,
                Err(_e) => {
                    eprint!("Failed to get document root for profile XML!");
                    continue;
                }
            };
            ...
            let auth_type = match traverse_xml_tree(&root, &["MSM", "security", "authEncryption", "authentication"]){
                Some(t) => t,
                None => {
                    eprint!("Failed to get the auth type for this profile!");
                    continue;
                }
            };
            ...
            match auth_type.as_str(){
                ...
                "WPA2" | "WPA2PSK" => {
                    if let Some(password) = traverse_xml_tree(&root, &["MSM", "security", "sharedKey", "keyMap"]){
